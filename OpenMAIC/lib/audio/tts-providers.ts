...
export async function generateTTS(
  ...
  switch (config.providerId) {
    ...
    case 'meituan-omni-tts':
      return await generateMeituanOmniTTS(config, text);
...
/**
 * Meituan Omni TTS (LongCat-Flash-Omni) implementation
 * Uses aigc.sankuai.com multimodal model that returns PCM audio URL,
 * then converts PCM (s16le 16kHz mono) to WAV in-memory.
 */
async function generateMeituanOmniTTS(
  config: TTSModelConfig,
  text: string,
): Promise<TTSGenerationResult> {
  const baseUrl = config.baseUrl || 'https://aigc.sankuai.com/v1/openai/native';
  const apiKey = config.apiKey;

  // Call LongCat-Flash-Omni with text+audio output
  ...
  if (!response.ok) {
    ...
    throw new Error(`Meituan Omni TTS API error: ${err}`);
  }

  const data = await response.json();
  const audioInfo = data?.choices?.[0]?.message?.audio;
  if (!audioInfo?.data) {
    throw new Error(`Meituan Omni TTS: no audio URL in response`);
  }

  // Download PCM audio
  const audioUrl = audioInfo.data as string;
  const audioRes = await fetch(audioUrl);
  if (!audioRes.ok) {
    throw new Error(`Meituan Omni TTS: failed to download audio`);
  }

  const pcmBuffer = await audioRes.arrayBuffer();
  const pcm = new Uint8Array(pcmBuffer);

  // Convert PCM s16le 16kHz mono → WAV
