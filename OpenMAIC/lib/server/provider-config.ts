...
const TTS_ENV_MAP: Record<string, string> = {
  ...
  TTS_GLM: 'glm-tts',
  TTS_QWEN: 'qwen-tts',
  TTS_MEITUAN_OMNI: 'meituan-omni-tts',
};
...
export function getServerProviders(): Record<string, { models?: string[]; baseUrl?: string; hasApiKey?: boolean }> {
  const cfg = getConfig();
  const result: Record<string, { models?: string[]; baseUrl?: string; hasApiKey?: boolean }> = {};
  for (const [id, entry] of Object.entries(cfg.providers)) {
    ...
    if (entry.apiKey) result[id].hasApiKey = true;
  }
  return result;
}
