...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Server-side model config: always use server env vars, ignore client model/key/baseUrl
    const serverModel = process.env.DEFAULT_MODEL || 'openai:gpt-4.1';
    const { providerId, modelId } = parseModelString(serverModel);

    const effectiveApiKey = resolveApiKey(providerId, undefined);
    const effectiveBaseUrl = resolveBaseUrl(providerId, undefined);
    const proxy = resolveProxy(providerId);

    if (!effectiveApiKey) {
      return apiError('MISSING_API_KEY', 401, 'Server API Key not configured');
    }
