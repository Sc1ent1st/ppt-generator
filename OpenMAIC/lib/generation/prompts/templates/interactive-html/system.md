...
- Use **inline CSS styles only** (no external CSS frameworks or CDN). Do NOT use Tailwind CSS CDN or any external stylesheets. Write all styles as `style="..."` attributes or a `<style>` block in `<head>`.
...
- Pure JavaScript only (no frameworks or external JS libraries). No CDN resources of any kind (no jsdelivr, unpkg, cdn.tailwindcss.com, etc).
...
- The HTML must be completely self-contained (no external resources, no CDN links of any kind)
