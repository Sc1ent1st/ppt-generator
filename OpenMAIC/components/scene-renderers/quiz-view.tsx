...
export function QuizView({ questions, sceneId }: QuizViewProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Notify teacher when quiz is completed
      try {
        const totalPts = questions.reduce((sum, q) => sum + (q.points ?? 1), 0);
        const earnedPts = ordered.reduce((sum, r) => sum + r.earned, 0);
        // Get classroomId from URL path /classroom/<id>
        const pathParts = window.location.pathname.split('/');
        const classroomIdx = pathParts.indexOf('classroom');
        const classroomId = classroomIdx >= 0 ? pathParts[classroomIdx + 1] : sceneId;
        // Get student MIS from URL query param ?mis=xxx
        const urlParams = new URLSearchParams(window.location.search);
        const studentMis = urlParams.get('mis') || 'wangjinyan08';
        fetch('/api/quiz-notify', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            classroomId,
            sceneId,
            studentId: studentMis,
